...
export default function FinanceChart() {
    ...
    const getFinancialTip = (actualSpending: number, spendingGoal: number) => {
        ...
        if (!actualSpending || !spendingGoal || isNaN(actualSpending) || isNaN(spendingGoal)) {
        ...
        if (percentage < 80) return "✅ Ótimo trabalho! Seus gastos estão bem controlados. \n💰Que tal investir a diferença ou reforçar sua reserva? 📈"
        if (percentage < 100) return "⚠️ Atenção! Você está se aproximando da meta. \n👀Reveja gastos com delivery, lazer ou compras. 📊";
    ...
    const getStyleClass = (actualSpending: number, spendingGoal: number) => {
        ...
        return styles.find(({ min, max }) => percentage >= min && percentage < max)!;
    };
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            ...
            const categories: Transaction[] = await activeAccount.getExpensesByCategoryForMonth(month, year);

            const chartData = categories.map(transaction => ({
                name: formatEnumName(transaction.subtype.split(":")[1]?.trim() || "No description"),
            ...
            setCategoryColors((prevColors) => {
                ...
                chartData.forEach((item, index) => {
                    if (!updatedColors[item.name]) {
                        updatedColors[item.name] = generateColorByIndex(index, chartData.length);
                    }
                });
             ...
             max-w-full sm:max-w-2xl mx-auto break-words"
                 ...
                 bg-yellow-50 text-yellow-800 flex items-center gap-2 shadow-sm break-words"
                                ...
                                (() => {
                                    ...
                                    const dica = getFinancialTip(monthlyTotal, Goal)?.toString() || "";
                                    return (
                                        <div
                                    ...
                                    <PieChart>
                                        <Pie
                                            ...
                                            label={({ name, value }) =>
                                                ...
                                                isMobile
                                                    ? `${(((value ?? totalAmount) / totalAmount) * 100).toFixed(0)}%`
                                                    : `${name.split(":")[1]?.trim() ?? name}: ${(((value ?? totalAmount) / totalAmount) * 100).toFixed(0)}%`
                                            }
                                            labelLine={false}
